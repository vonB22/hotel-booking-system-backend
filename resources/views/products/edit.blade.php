...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a class="btn btn-outline-secondary btn-sm shadow-sm" href="{{ route('products.index') }}">
            ...
            <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" value="{{ $product->name }}" class="form-control" placeholder="e.g., Grand Palace Hotel" required>
                    </div>

                    <!-- Location -->
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Location</label>
                        <input type="text" name="location" value="{{ old('location', $product->location ?? '') }}" class="form-control" placeholder="e.g., Paris, France">
                    </div>

                    <!-- Price per Night -->
                    <div class="col-md-4">
                        <label class="form-label fw-semibold">Price per Night ($)</label>
                        <input type="number" name="price" value="{{ old('price', $product->price ?? '') }}" class="form-control" placeholder="e.g., 120">
                    </div>

                    <!-- Available Rooms -->
                    <div class="col-md-4">
                        <label class="form-label fw-semibold">Available Rooms</label>
                        <input type="number" name="rooms" value="{{ old('rooms', $product->rooms ?? '') }}" class="form-control" placeholder="e.g., 20">
                    </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="rating" class="form-select">
                            ...
                            @for ($i = 1; $i <= 5; $i++)
                                <option value="{{ $i }}" {{ (isset($product->rating) && $product->rating == $i) ? 'selected' : '' }}>
                                    {{ str_repeat('⭐', $i) }}
                                </option>
                            @endfor
                        </select>
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea class="form-control" name="detail" rows="4" placeholder="Describe the hotel...">{{ $product->detail }}</textarea>
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="row">
                            @php
                                ...
                                $selectedAmenities = is_array($product->amenities ?? null)
                                    ? $product->amenities
                                    : explode(',', $product->amenities ?? '');
                            @endphp
                    ...
                    <div class="col-md-12">
                        ...
                        @if (!empty($product->image))
                            <img src="{{ asset('storage/' . $product->image) }}" alt="Hotel Image" class="rounded mt-2" width="160">
                        @endif
                    ...
                    <div class="col-md-12 text-center mt-4">
                        ...
                        <a href="{{ route('products.show', $product->id) }}" class="btn btn-outline-info btn-lg px-4 ms-2">
