...
<div class="container-fluid">
    ...
    <div class="card shadow-sm border-0 rounded-3 overflow-hidden">
        @if(!empty($product->image))
            <img src="{{ asset('storage/' . $product->image) }}" class="card-img-top" alt="{{ $product->name }}" style="object-fit: cover; height: 320px;">
        @else
        ...
        <div class="card-body p-4">
            ...
            <div class="mb-3 text-muted small">
                ...
                <strong>{{ $product->price ?? 'N/A' }}</strong> per night
            </div>

            <!-- Hotel Description -->
            <div class="mb-4">
                <h6 class="fw-semibold text-primary">Description</h6>
                <p class="text-muted mb-0">{{ $product->detail ?? 'No details available.' }}</p>
            </div>

            <!-- Amenities -->
            @if(!empty($product->amenities))
                @php
                    $amenities = is_array($product->amenities)
                        ? $product->amenities
            ...
            <div class="mb-4">
                ...
                <ul class="list-unstyled text-muted mb-0">
                    <li><i class="fa-solid fa-bed me-2 text-primary"></i> Available Rooms: {{ $product->rooms ?? 'N/A' }}</li>
                    <li><i class="fa-solid fa-star me-2 text-warning"></i> Rating: {{ $product->rating ?? 'N/A' }} / 5</li>
                </ul>
            </div>

            <!-- Action Buttons -->
            <div class="text-center mt-4">
                <a href="{{ route('products.edit', $product->id) }}" class="btn btn-primary btn-lg px-4 shadow-sm me-2">
